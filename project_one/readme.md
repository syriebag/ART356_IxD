# Project One: FUNity
🪞 [Link to FUNity](https://syriebag.github.io/ART356_IxD/project_one/index.html)

This project was inspired by Professor Tyler's comment about his appreciation of students having profile pictures in Canvas. 
With this, I thought, what if there's an avatar random generator that people can screenshot and use as a profile picture?

## Wireframe
|**version 1**|**version 2**|
|:---:|:---:|
| <img width="500" height="350" alt="Screenshot 2026-09-21 at 9 21 21 PM" src="https://github.com/user-attachments/assets/dec89fd8-08fa-4c8e-b082-698ffe525b67" /> | <img width="500" height="350" alt="Screenshot 2026-09-21 at 9 22 12 PM" src="https://github.com/user-attachments/assets/669d5649-29fc-4f86-8371-aa47aa92a568" /> |
| The original idea was to take user input of their name and randomly generate a face. I thought this idea was funny and engaging since the output is based on their name, their identity, and will produce either a positive or negative response.
Eventually leading them to input others' names and make a comparison. However, due to time constraints and programming complexity, I resorted to plan B. | This wireframe, version 2, is visibly inspired by the layout and the background/graphics I had made. This version is a simple interaction with users by clicking the start button to run the program.  |


## Moodboard
<img width="996" height="708" alt="Screenshot 2026-09-21 at 9 21 54 PM" src="https://github.com/user-attachments/assets/84bfe1e8-15ca-4d9d-880d-b60d06f8eb66" />
